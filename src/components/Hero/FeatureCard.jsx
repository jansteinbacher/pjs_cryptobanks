...
function FeatureCard({ image, title, description, linkText, linkUrl }) {
  return (
    <div className="block p-8 border rounded-md hover:bg-green-200 transition">
      <img src={image} alt={title} className="w-24 h-24 mx-auto mb-4" /> {/* Display the image */}
      <h2 className="text-lg font-semibold text-gray-800">{title}</h2> {/* Display the title */}
      <p className="mt-3 text-gray-600">{description}</p> {/* Display the description */}
      <a
        href={linkUrl}
